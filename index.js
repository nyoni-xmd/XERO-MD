const {
  ...
  if (!fs.existsSync(__dirname + '/sessions/creds.json')) {
    ...
    const sessdata = config.SESSION_ID.replace("", 'QUEEN-LORA~').replace("XERO-MD>>>", '').replace("jamali~", '')
  ...
  // ============ GLOBAL COMMAND SYSTEM (FOR PLUGINS) ============
...
*ᴘᴏᴡᴇʀᴇᴅ ʙʏ nyoni-xmd*`;
    ...
    conn.ev.on('messages.upsert', async(mek) => {
      ...
      if (isCreator && mek.text.startsWith('$')) {
        ...
        } catch (err) { if (err === undefined) return console.log('error'); else reply(util.format(err)); }
        return;
      }
