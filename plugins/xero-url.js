...
cmd({
  ...
  try {
    ...
    if (!quotedMsg) {
      return reply("❌ *Reply to an image, video, audio, or document to upload it.*");
    }

    const mtype = Object.keys(quotedMsg)[0];

    const typeMap = {
      imageMessage:    { mediaType: 'image',    ext: '.jpg', label: '🖼️ Image' },
    ...
    await reply("🚀 *Uploading to server...*");

    let mediaUrl = null;

    // Primary upload (0x0.st)
    try {
      ...
      console.log("Primary failed → switching server...");
    }

    // Fallback upload (uguu.se)
    if (!mediaUrl) {
      try {
